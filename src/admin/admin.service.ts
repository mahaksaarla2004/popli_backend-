...
export class AdminService {
  ...
  async login(email: string, passwordString: string) {
    ...
    if (!admin) {
      ...
      const existingAdminsCount = await this.prisma.user.count({ where: { role: 'ADMIN' } });
      if (existingAdminsCount === 0 && email === 'admin@popli.com' && passwordString === 'admin123') {
        ...
        return { token, user: { id: newAdmin.id, name: newAdmin.name, email: newAdmin.email } };
      }
  ...
  async getDashboardStats(adminId: string) {
    ...
    const totalCreators = await this.prisma.user.count({ where: { role: 'CREATOR' } });
  ...
  async getReels(adminId: string) {
    ...
    return this.prisma.reel.findMany({
      include: { creator: { select: { username: true, name: true, avatar: true } } },
      orderBy: { createdAt: 'desc' }
    });
  }
  ...
  async getTransactions(adminId: string) {
    ...
    return this.prisma.transaction.findMany({
      include: { wallet: { include: { user: { select: { username: true, name: true } } } } },
      orderBy: { createdAt: 'desc' }
    });
  }
  ...
  async getReports(adminId: string) {
    ...
    return this.prisma.report.findMany({
      include: { 
        ...
        reel: { select: { description: true, creator: { select: { username: true } } } }
      },
      orderBy: { createdAt: 'desc' }
    });
  }
