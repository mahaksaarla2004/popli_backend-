...
export class ReelsService {
  ...
  async getNearbyFeed(lat: number, lng: number, radiusKm: number = 50, page: number = 1, limit: number = 10) {
  ...
  async toggleLike(reelId: string, userId: string) {
    ...
    if (existingLike) {
      ...
      const reel = await this.prisma.reel.update({ where: { id: reelId }, data: { likesCount: { decrement: 1 } } });
      await this.prisma.user.update({ where: { id: reel.creatorId }, data: { totalLikesReceived: { decrement: 1 } } });
      return { liked: false };
    } else {
      await this.prisma.like.create({ data: { reelId, userId } });
      const reel = await this.prisma.reel.update({ where: { id: reelId }, data: { likesCount: { increment: 1 } } });
      await this.prisma.user.update({ where: { id: reel.creatorId }, data: { totalLikesReceived: { increment: 1 } } });
  ...
  async toggleSave(reelId: string, userId: string) {
    ...
    if (existingSave) {
      ...
      await this.prisma.reel.update({ where: { id: reelId }, data: { savesCount: { decrement: 1 } } });
      return { saved: false };
    } else {
      await this.prisma.save.create({ data: { reelId, userId } });
      await this.prisma.reel.update({ where: { id: reelId }, data: { savesCount: { increment: 1 } } });
      return { saved: true };
    }
  }
  ...
  async addComment(reelId: string, userId: string, dto: AddCommentDto) {
    ...
    const reel = await this.prisma.reel.update({ where: { id: reelId }, data: { commentsCount: { increment: 1 } } });
  ...
  async toggleCommentLike(commentId: string, userId: string) {
    ...
    if (existingLike) {
      ...
      const comment = await this.prisma.comment.update({ where: { id: commentId }, data: { likesCount: { increment: 1 } } });
      const user = await this.prisma.user.findUnique({ where: { id: userId } });
