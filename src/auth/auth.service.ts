...
import { SendOtpDto, VerifyOtpDto, RefreshTokenDto, VerifyFirebaseTokenDto } from './dto/auth.dto';
...
export class AuthService {
  ...
  async verifyFirebaseToken(dto: VerifyFirebaseTokenDto, ip: string, userAgent: string) {
  ...
  async verifyOtp(dto: VerifyOtpDto, ip: string, userAgent: string) {
    ...
    throw new BadRequestException('Please use Firebase Token Verification for OTPs.');
    ...
    if (user && dto.username) {
      throw new BadRequestException('Account already exists with this mobile number. Please log in.');
    }
  ...
  async refreshToken(dto: RefreshTokenDto) {
    ...
    try {
      this.jwtService.verify(dto.refreshToken, { secret: process.env.JWT_REFRESH_SECRET });
    } catch {
